(function(root){
  ...
  function comparison(r,c){
    ...
    return `<div class="recipe-comparison" aria-label="Before and after computational preview"><div><strong>Before — ATLAS Target</strong><div class="recipe-swatch" style="background:${esc(c.hex)}" role="img" aria-label="Before, ATLAS target ${esc(c.hex)}"></div><code>${esc(c.hex)}</code></div><div><strong>After — Computed Mix</strong><div class="recipe-swatch" style="background:${r.mix_display.hex}" role="img" aria-label="After, computed mix ${r.mix_display.hex}"></div><code>${r.mix_display.hex}</code></div></div><p class="recipe-preview-boundary">COMPUTATIONAL PREVIEW · NOT PHYSICALLY VERIFIED</p><p class="recipe-note">Screen proxies. ΔE00 compares model spectra under D50 / 2°, not these displayed RGB values.${r.mix_display.srgb_clipped?' Computed mix clipped to sRGB.':''}</p><button type="button" data-recipe-png>Download comparison PNG</button>`;
  }
