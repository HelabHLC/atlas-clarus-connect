...
check(invoke_private('validate_manifest', $manifest) === true, 'Pinned RC23 manifest rejected');
...
$fault = '';
check(invoke_private('install_bundle') === true, 'RC23 installation failed');
...
check($options[ATLAS_Clarus_Browser_Edition::OPTION_PREVIOUS_RUNTIME] === $previous, 'Repeated install replaced rollback with RC23');
check(is_file($runtime . '/index.html') && is_file($old . '/index.html'), 'Repeat install removed existing runtime');

// Real RC23 document: navigation injection must preserve application script bytes.
...
register_shutdown_function(function () use ($old) {
    ...
    echo "PASS: RC23 PHP manifest, guarded mutations, failure preservation, install, repeat install, canvas injection and rollback\n";
});
$finished = true;
