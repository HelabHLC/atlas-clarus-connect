...
check(invoke_private('validate_manifest', $manifest) === true, 'Pinned RC24 manifest rejected');
...
$fault = '';
check(invoke_private('install_bundle') === true, 'RC24 installation failed');
...
check($options[ATLAS_Clarus_Browser_Edition::OPTION_PREVIOUS_RUNTIME] === $previous, 'Repeated install replaced rollback with RC24');
check(is_file($runtime . '/index.html') && is_file($old . '/index.html'), 'Repeat install removed existing runtime');

// Real RC24 document: navigation injection must preserve application script bytes.
...
register_shutdown_function(function () use ($old) {
    ...
    echo "PASS: RC24 PHP manifest, guarded mutations, failure preservation, install, repeat install, canvas injection and rollback\n";
});
$finished = true;
