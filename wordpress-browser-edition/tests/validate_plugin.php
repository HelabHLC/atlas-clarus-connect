...
check(invoke_private('validate_manifest', $manifest) === true, 'Pinned RC27 manifest rejected');
...
check($options[ATLAS_Clarus_Browser_Edition::OPTION_PREVIOUS_RUNTIME] === $previous, 'Repeated install replaced rollback with RC27');
check(is_file($runtime . '/index.html') && is_file($old . '/index.html'), 'Repeat install removed existing runtime');

// Real RC27 document: navigation injection must preserve application script bytes.
...
register_shutdown_function(function () use ($old) {
    ...
    echo "PASS: RC27 PHP manifest, guarded mutations, failure preservation, install, repeat install, canvas injection and rollback\n";
});
$finished = true;
