...
The connector is manufacturer-neutral. It transports an APF–AxF identity
binding to a renderer and binds the returned bytes as a render-result asset.
It does not make an external renderer conformant with AxF.
...
- `asset`: AxF identity, URI, digest, media declaration and, in external
...
- `mock`: public deterministic connector test, no AxF decoding.
