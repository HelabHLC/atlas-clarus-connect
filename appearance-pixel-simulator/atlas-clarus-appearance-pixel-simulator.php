...
define( 'ATLAS_CLARUS_APS_VERSION', '0.2.2' );
			...
			<table class="form-table" role="presentation">
				...
				<tr>
					...
					<td>
						...
						<p class="description"><?php esc_html_e( 'Mock mode validates the connector and evidence chain but does not decode AxF.', 'atlas-clarus-appearance-pixel-simulator' ); ?></p>
					</td>
				</tr>
				<tr>
...
function atlas_clarus_aps_mock_render( $request ) {
	...
	return array(
		...
		'limitation'         => 'Connector test only. AxF bytes were not decoded and no physical measurement was performed.',
	);
}
...
function atlas_clarus_aps_renderer_request( WP_REST_Request $request ) {
	...
	if ( ATLAS_CLARUS_APS_MASTER_SHA256 !== $payload['identity']['master_sha256'] || ! preg_match( '/^[a-f0-9]{64}$/', $payload['asset']['sha256'] ) ) {
		return new WP_Error( 'atlas_clarus_invalid_render_identity', 'Master or AxF digest validation failed.', array( 'status' => 422 ) );
	}
	...
	if ( empty( $payload['asset']['content_base64'] ) ) {
		return new WP_Error( 'atlas_clarus_renderer_asset_missing', 'External mode requires AxF asset bytes.', array( 'status' => 422 ) );
	}
	$axf_bytes = base64_decode( $payload['asset']['content_base64'], true );
	if ( false === $axf_bytes || strlen( $axf_bytes ) > 10 * MB_IN_BYTES || ! hash_equals( $payload['asset']['sha256'], hash( 'sha256', $axf_bytes ) ) ) {
		return new WP_Error( 'atlas_clarus_renderer_asset_integrity', 'AxF bytes are invalid, exceed 10 MiB or do not match the declared digest.', array( 'status' => 422 ) );
	}
