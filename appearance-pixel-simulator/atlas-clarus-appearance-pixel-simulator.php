		...
		<form action="options.php" method="post">
			...
			<table class="form-table" role="presentation">
				...
				<tr>
					...
					<td>
						<select id="atlas-clarus-aps-renderer-mode" name="atlas_clarus_aps_renderer_mode"><option value="mock" <?php selected( get_option( 'atlas_clarus_aps_renderer_mode', 'mock' ), 'mock' ); ?>><?php esc_html_e( 'Colour-aware mock simulation', 'atlas-clarus-appearance-pixel-simulator' ); ?></option><option value="external" <?php selected( get_option( 'atlas_clarus_aps_renderer_mode', 'mock' ), 'external' ); ?>><?php esc_html_e( 'External MaterialX JSON renderer', 'atlas-clarus-appearance-pixel-simulator' ); ?></option></select>
						<p class="description"><?php esc_html_e( 'Mock mode is local and requires no rendering server. External rendering is optional and requires a user- or organization-operated compatible endpoint. ATLAS Clarus does not provide a hosted renderer.', 'atlas-clarus-appearance-pixel-simulator' ); ?></p>
					</td>
				</tr>
				<tr>
					...
					<td><input class="regular-text" id="atlas-clarus-aps-renderer-endpoint" name="atlas_clarus_aps_renderer_endpoint" type="url" value="<?php echo esc_attr( get_option( 'atlas_clarus_aps_renderer_endpoint', '' ) ); ?>" placeholder="https://your-renderer.example/v1/render"><p class="description"><?php esc_html_e( 'Optional bring-your-own-renderer endpoint controlled by you or your chosen service provider.', 'atlas-clarus-appearance-pixel-simulator' ); ?></p></td>
				</tr>
				<tr>
...
function atlas_clarus_aps_mock_render( $request ) {
	...
	$base_hex = isset( $request['display_proxy']['master_hex'] ) ? strtoupper( sanitize_text_field( $request['display_proxy']['master_hex'] ) ) : '#808080';
	if ( ! preg_match( '/^#[0-9A-F]{6}$/', $base_hex ) ) {
		$base_hex = '#808080';
	}
	$highlight_x = max( 0.15, min( 0.85, $angle / 90 ) );
	$svg = '<svg xmlns="http://www.w3.org/2000/svg" width="960" height="600" viewBox="0 0 960 600">'
		. '<defs><radialGradient id="illustrativeGloss" cx="' . esc_attr( (string) $highlight_x ) . '" cy=".30" r=".72"><stop stop-color="#fff" stop-opacity=".78"/><stop offset=".18" stop-color="#fff" stop-opacity=".24"/><stop offset=".48" stop-color="#fff" stop-opacity="0"/><stop offset="1" stop-color="#000" stop-opacity=".22"/></radialGradient></defs>'
		. '<rect width="960" height="600" fill="#172330"/>'
		. '<rect x="74" y="64" width="812" height="420" rx="28" fill="' . esc_attr( $base_hex ) . '" stroke="#fff" stroke-opacity=".55" stroke-width="2"/>'
		. '<rect x="74" y="64" width="812" height="420" rx="28" fill="url(#illustrativeGloss)"/>'
		. '<rect x="98" y="88" width="360" height="52" rx="12" fill="#101820" fill-opacity=".90"/>'
		. '<text x="118" y="122" fill="#fff" font-family="sans-serif" font-size="22" font-weight="700">GENERIC MOCK PREVIEW</text>'
		. '<text x="74" y="530" fill="#fff" font-family="sans-serif" font-size="23">' . esc_html( $label ) . ' · ' . esc_html( $base_hex ) . '</text>'
		. '<text x="74" y="565" fill="#fff" fill-opacity=".72" font-family="sans-serif" font-size="17">MASTER COLOUR PROXY · ILLUSTRATIVE GLOSS · NO MATERIALX DECODING · QC NOT_MEASURED</text>'
		. '</svg>';
	return array(
		...
		'limitation'         => 'Colour-aware generic connector preview using the bound master display proxy. Gloss is illustrative; material bytes were not decoded and no physical measurement was performed.',
		'display_proxy_hex'  => $base_hex,
	);
}
			...
			<div class="atlas-clarus-aps__stage">
				...
				<details class="atlas-clarus-aps__bridge">
					...
					<div class="atlas-clarus-aps__actions">
						...
						<button type="button" data-bridge-action="validate"><?php esc_html_e( 'Import & validate bridge JSON', 'atlas-clarus-appearance-pixel-simulator' ); ?></button>
